...
if (isset($_GET['start']) && isset($_GET['end'])) {
  ...
  $arr = array();
  $returnCal = $conn->query("SELECT tankkeeping_entries.timestamp, tankkeeping_entries.note, tankkeeping_types.`cal-color`, tankkeeping_types.type FROM `tankkeeping_entries` INNER JOIN tankkeeping_types ON tankkeeping_entries.type_id = tankkeeping_types.id WHERE tankkeeping_entries.timestamp >= '".$start."' AND tankkeeping_entries.timestamp <= '".$end."'");
