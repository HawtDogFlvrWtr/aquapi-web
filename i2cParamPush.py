...
i2cDict = { 97: "DO", 98: "ORP", 99: "PH", 100: "Salinity", 102: "Temperature", 103: "PMP" }
...
def main():
	...
	for i in range(len (devices)):
            print("Working on: "+str(i2cDict[devices[i]]))
	    device.set_i2c_address(devices[i])
	    returnVal = device.query("R")
            print returnVal
            if returnVal != "Error":
                try:
                    sendVal = urllib2.urlopen("http://localhost/api/pushValue.php?metric="+str(i2cDict[devices[i]])+"&value="+str(returnVal)).read()
                    print sendVal
                except:
